...
const isPageVisibilitySupported = () =>
  hidden && typeof document[hidden] !== "undefined";
export const isPageVisible = () => {
  if (!isPageVisibilitySupported()) return true;
...
if (!isPageVisibilitySupported()) {
  console.log(
    'Page visibility API seems to be unsupported, page will always count as "visible".'
  );
}
