...
type TriggerEngagementMessageAction = ReturnType<
  typeof triggerEngagementMessage
>;
  ...
  switch (message.source) {
    case "agent":
      return (
        ...
        state.config.settings.agentAvatarUrl ||
        defaultAgentAvatar
      );
    case "bot":
    case "engagement":
    ...
    case "user":
      return (
        state.ui.userAvatarOverrideUrl ||
        state.config.settings.userAvatarUrl ||
        defaultUserAvatar
      );
  }
};
    ...
    switch (action.type) {
      ...
      case "RECEIVE_MESSAGE": {
        ...
        const isMessageEmpty = !(
          message.text || message.data?._cognigy?._webchat
        );
