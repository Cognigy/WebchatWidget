	...
	(action: Actions) => {
		switch (action.type) {
			case "SWITCH_SESSION": {
				...
				client.switchSession(sessionId || ""); // note that sessionId, if empty string given, is generated by socket-client
				store.dispatch(setPrevState(conversation || { messages: [], rating: ratingInitialState }));
				break;
			}
			case "SEND_MESSAGE":
