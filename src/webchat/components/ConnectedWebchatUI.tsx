...
} from "../store/messages/message-middleware";
import {
  setInputMode,
...
export type FromProps = Pick<
  WebchatUIProps,
  "messagePlugins" | "inputPlugins" | "webchatRootProps" | "webchatToggleProps"
>;
type Merge = FromState &
  FromDispatch &
  FromProps &
  Pick<WebchatUIProps, "fullscreenMessage">;

export const ConnectedWebchatUI = connect<
  FromState,
  FromDispatch,
  FromProps,
  Merge,
...
>(
  ...
  (dispatch) => ({
    onSendMessage: (text, data, options) =>
      dispatch(sendMessage({ text, data }, options)),
    onSetInputMode: (inputMode) => dispatch(setInputMode(inputMode)),
    onSetFullscreenMessage: (message) =>
      dispatch(setFullscreenMessage(message)),
  ...
  ({ fullscreenMessage, ...state }, dispatch, props) => {
    if (!fullscreenMessage) {
      ...
      const matchedPlugins = lastMessage
        ? getPluginsForMessage(
            props.messagePlugins || [],
            state.config
          )(lastMessage)
        : [];
