...
class RatingDialog extends React.PureComponent<
  React.HTMLProps<HTMLDivElement> & IRatingDialogProps,
  IRatingDialogState
> {
  ...
  restoreFocus = () => {
    const thumdsUpDownButton = document.getElementById(
  ...
  handleKeydown = (event) => {
    const isSendButtonDisabled =
      this.state.ratingValue !== -1 && this.state.ratingValue !== 1;
     ...
     */
    if (
      event.target === this.sendRatingButtonRef?.current ||
      (isSendButtonDisabled &&
        event.target === this.commentTextAreaRef?.current)
    ) {
      if (event.shiftKey && event.key === "Tab") {
  ...
  render() {
    ...
    const { ratingValue, ratingText } = state;
    ...
    return (
      <Wrapper>
        ...
        >
          <RatingDialogHeader>
            <span id={webchatRatingDialogTitleId} role="heading" aria-level={2}>
              {ratingTitleText}
            </span>
            <HeaderIconButton
