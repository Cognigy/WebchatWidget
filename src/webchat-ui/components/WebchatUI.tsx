...
> {
  ...
  private unreadTitleIndicatorInterval: ReturnType<typeof setInterval> | null =
    null;
  ...
  static getDerivedStateFromProps(
    ...
    const color =
      ...
      props.config.settings.colorScheme;
  ...
  componentDidUpdate(prevProps: WebchatUIProps, prevState: WebchatUIState) {
    if (
      this.props.config.settings.colorScheme !==
      prevProps.config.settings.colorScheme
    ) {
      this.setState({
        theme: createWebchatTheme({
          primaryColor: this.props.config.settings.colorScheme,
        }),
      });
    }
    ...
    if (
      prevProps.unseenMessages !== this.props.unseenMessages ||
      ...
      if (
        ...
        this.props.config.settings.enableUnreadMessageSound
      ) {
    ...
    if (
      ...
      this.props.config.settings.enableUnreadMessageTitleIndicator !==
        prevProps.config.settings.enableUnreadMessageTitleIndicator
    ) {
      this.initializeTitleIndicator();
    }
  ...
  initializeTitleIndicator = () => {
    if (this.unreadTitleIndicatorInterval) return;

    this.unreadTitleIndicatorInterval = setInterval(
      this.toggleTitleIndicator,
      1000
    );
  };
  ...
  handleReverseTabNavigation = () => {
    const webchatHistoryPanel = document.getElementById(
    ...
    const webchatInputButtonMenu = document.getElementById(
      "webchatInputButtonMenu"
    );
    if (textMessageInput) {
  ...
  handleKeydown = (event) => {
    ...
    if (enableFocusTrap && open) {
       ...
       *
       */
      const isCloseButtonAsTarget =
        event.target === this.closeButtonInHeaderRef?.current;
      const isRatingButtonAsTarget =
        event.target === this.ratingButtonInHeaderRef?.current;
      if (
        (!enableRating && isCloseButtonAsTarget) ||
        (enableRating && isRatingButtonAsTarget)
      ) {
      ...
      if (event.target === this.chatToggleButtonRef?.current) {
        if (event.shiftKey && event.key === "Tab") {
          ...
          const hasRatingButton =
            enableRating &&
            (enableRating === "always" ||
              (enableRating === "once" && this.props.hasGivenRating === false));
          if (hasRatingButton) {
            event.preventDefault();
            const webchatHeaderRatingButton = document.getElementById(
  ...
  render() {
    ...
    if (!this.props.config.active) return null;
  ...
  renderRegularLayout() {
    ...
    const showRatingButton =
      enableRating &&
      (enableRating === "always" ||
        (enableRating === "once" && hasGivenRating === false));
