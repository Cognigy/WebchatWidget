...
export const getSpeechInput = ({ React, styled }: InputPluginFactoryProps) => {
  ...
  return class SpeechInput extends React.Component<
    InputComponentProps,
    ISpeechInputState
  > {
    ...
    componentDidUpdate() {
      if (
        ...
        this.state.speechRecognition.lang =
          this.props.config.settings.STTLanguage;
      }
    }
    ...
    toggle = () => {
      ...
      if (active) {
        (newState.result = ""), (newState.isFinalResult = false);
      }
    ...
    renderMicIcon() {
      if (!this.isSupported()) return <MicNoneIcon />;

      if (this.state.active) return <MicOffIcon />;
    ...
    render() {
      ...
      const buttonClasses = [active && "right"]
        .filter((name) => typeof name === "string")
        .join(" ");

      const inputClasses = [!isFinalResult && "interim"]
        .filter((name) => typeof name === "string")
        .join(" ");

      const wrapperClasses = [active && "active"]
        .filter((name) => typeof name === "string")
        .join(" ");

      return (
        <Wrapper className={wrapperClasses}>
          <Input className={inputClasses} disabled value={result} />
          <Button
