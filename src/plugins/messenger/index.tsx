...
const isMessengerPayload = (message: IMessage, config: IWebchatConfig) =>
  !!getMessengerPayload(message, config);
...
const isFullscreenMessengerGenericPayload = (
  rawMessage: IMessage,
  config: IWebchatConfig
) => {
  const messengerPayload = getMessengerPayload(rawMessage, config);

  if (!messengerPayload) return false;
  ...
  if (payload.template_type !== "generic") return false;
      ...
      <MessengerPreview
        ...
        onAction={(e, action) => {
          ...
          if (action.type === "web_url" && action.url) {
            ...
            const target =
              (action as IFBMURLButton).target === "_self" ? "_self" : "_blank";
...
const fullscreenMessengerGenericPlugin: MessagePluginFactory = ({
  React,
  styled,
}) => {
  const MessengerPreview = getMessengerPreview({ React, styled });
      ...
      <MessengerPreview
        ...
        onAction={(e, action) => {
          ...
          if (action.type === "web_url") {
            ...
            window.open(
              action.url,
              action.target === "_self" ? "_self" : "_blank"
            );
          }
        }}
        config={config}
