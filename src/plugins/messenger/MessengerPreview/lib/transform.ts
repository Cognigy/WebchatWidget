...
const transformAttachmentUploadApiMessage = (
  message: IFBMMessage | IFBMUploadAPIMessage
): IFBMMessage => {
  const { attachment } = message as IFBMAttachmentMessage;

  // regular text message
  if (!attachment) return message as IFBMMessage;

  // regular template message
  if (attachment.type === "template") return message as IFBMMessage;

  const {
    type: media_type,
    payload: { url, altText },
  } = attachment as unknown as IFBMUploadAPIMessage["attachment"];
...
const filterButtons =
  (filter: ButtonFilter): IFBMTransformer =>
  (message) => {
    const { attachment } = message as IFBMAttachmentMessage;
    if (attachment) {
...
const removeUnsupportedButtons = filterButtons(
  (button) => button.type !== "element_share"
);

type QuickReplyFilter = (quickReply: IFBMQuickReply) => boolean;
const filterQuickReplies =
  (filter: QuickReplyFilter): IFBMTransformer =>
  (message) => {
...
const removeEmptyQuickReplies = filterQuickReplies(
  (q) => !(q.content_type === "text" && !q.title)
);
