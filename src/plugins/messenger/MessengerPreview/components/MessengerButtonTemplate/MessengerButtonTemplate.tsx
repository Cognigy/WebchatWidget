...
interface IMessengerButtonTemplateProps
...
export const getMessengerButtonTemplate = ({
  ...
  const MessengerButtonTemplate = ({
    ...
    const webchatButtonTemplateButtonId = useRandomId(
      "webchatButtonTemplateButton"
    );
    const webchatButtonTemplateTextId = useRandomId(
      "webchatButtonTemplateHeader"
    );
    const buttonGroupAriaLabelledby = text
      ? webchatButtonTemplateTextId
      : undefined;
    const a11yProps =
      buttons?.length > 1
    ...
    useEffect(() => {
      const firstButton = document.getElementById(
        `${webchatButtonTemplateButtonId}-0`
      );
    ...
    return (
      <MessengerButtonHeader
        {...divProps}
        color={color}
        className="webchat-buttons-template-root"
      >
        {text && (
          <Text
            className="webchat-buttons-template-header"
