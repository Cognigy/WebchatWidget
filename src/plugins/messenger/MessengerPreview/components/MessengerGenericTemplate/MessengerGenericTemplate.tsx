...
export const getMessengerGenericTemplate = ({
  ...
  > {
    ...
    isLegacyEdgeBrowser: boolean =
      window.navigator.userAgent.indexOf("Edge/") > 0; //to detect Edge 15 to 18
    ...
    componentDidMount() {
      ...
      const firstCardContent = document.getElementById(
        `${this.carouselContentId}-0`
      );
      const firstButton = document.getElementById(
        `${this.carouselButtonId}-00`
      );
    ...
    handleArrowKeyDown = (event) => {
      if (!this.isIE11Browser && !this.isLegacyEdgeBrowser) {
        ...
        if (
          ...
          selectedItem < lastPosition
        ) {
          this.setState({ selectedItem: selectedItem + 1 }, () => {
            this.focusCardInView();
          });
        } else if (
          (event.key === "ArrowLeft" || event.keyCode === "39") &&
          selectedItem > 0
        ) {
          this.setState({ selectedItem: selectedItem - 1 }, () => {
            this.focusCardInView();
    ...
    focusCardInView = () => {
      const nextCardToFocus = document.getElementById(
        `${this.carouselRootId}-${this.state.selectedItem}`
      );
    ...
    renderElement = (element: IFBMGenericTemplateElement, index?: number) => {
      ...
      const { image_url, image_alt_text, title, subtitle, default_action } =
        element;
      ...
      const buttonsCountLabel =
        buttons.length === 1
          ? "1 button or link."
          : `${buttons.length} buttons or links.`;
      const buttonsInListItemAriaLabel =
        buttons.length > 0 ? `with ${buttonsCountLabel}` : undefined;
      // When gallery items count is one, refer the gallery card as 'slide' instead of 'list item', in aria-label
      const listItemCount =
        index !== undefined
          ? `${carouselListLength === 1 ? "slide" : ""} ${
              index + 1
            } of ${carouselListLength} ${buttonsInListItemAriaLabel || ""}`
          : undefined;
      const carouselAriaLabel = title
        ? `${listItemCount}: ${title}`
        : listItemCount;
      ...
      const carouselRootA11yProps = {
        role: "group",
        "aria-labelledby": listItemCount ? undefined : carouselAriaLabelledby,
        "aria-label": listItemCount ? carouselAriaLabel : undefined,
        "aria-describedby": carouselAriaDescribedby,
      };
      const carouselTitle = title ? title + ". " : "";
      const carouselHeaderA11yProps = default_action?.url
        ? { "aria-label": carouselTitle + "Opens in new tab" }
      ...
      const titleHtml = this.props.config.settings
        .disableHtmlContentSanitization
        ? title
        : sanitizeHTML(title);
      const subtitleHtml = this.props.config.settings
        .disableHtmlContentSanitization
        ? subtitle
      ...
      return (
        <ElementRoot key={index} className="webchat-carousel-template-root">
          <Frame
            className={`webchat-carousel-template-frame ${
              isCentered ? "wide" : ""
            }`}
