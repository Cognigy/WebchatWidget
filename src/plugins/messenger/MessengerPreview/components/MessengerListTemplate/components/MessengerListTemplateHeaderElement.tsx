...
export const getMessengerListTemplateHeaderElement = ({
  ...
  const ListHeaderButton = styled(ListButton)<IWithMessageColor>(
    ({ theme, messageColor }) => ({
      backgroundColor:
        messageColor === "neutral" ? theme.greyColor : theme.primaryColor,
      color:
      ...
      "&:focus": {
        backgroundColor:
          messageColor === "neutral"
            ? theme.greyStrongColor
            : theme.primaryStrongColor,
      },
  ...
  const MessengerListTemplateHeaderElement = ({
    ...
    const ariaLabelForMessengerTitle = default_action?.url
      ...
      : title;
    const messengerSubtitleId = useRandomId(
      "webchatListTemplateHeaderSubtitle"
    );
    ...
    const image = config.settings.dynamicImageAspectRatio ? (
      ...
      >
        <span role="img" aria-label={image_alt_text || "List Image"}>
          {" "}
        </span>
      </FixedImage>
    );
    ...
    const buttonHtml =
      !!button &&
      (isSanitizeEnabled
        ? sanitizeHTML(getButtonLabel(button))
