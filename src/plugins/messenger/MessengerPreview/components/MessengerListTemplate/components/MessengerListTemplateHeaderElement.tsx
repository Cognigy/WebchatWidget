...
interface IMessengerListTemplateHeaderElementProps
...
export const getMessengerListTemplateHeaderElement = ({
	React,
	styled,
	...
	const ListHeaderButton = styled(ListButton)<IWithMessageColor & { isSelected?: boolean }>(
		({ theme, messageColor, isSelected }) => ({
			backgroundColor: messageColor === "neutral" ? theme.greyColor : theme.primaryColor,
			color:
				messageColor === "neutral" ? theme.greyContrastColor : theme.primaryContrastColor,
			border: `none`,
			cursor: "pointer",
			outline: "none",
			...
			"&:focus": {
				backgroundColor:
					messageColor === "neutral" ? theme.greyStrongColor : theme.primaryStrongColor,
			},
	...
	const MessengerListTemplateHeaderElement = ({
		element,
		onAction,
		config,
		messageColor,
		selectedOptionIndex,
		buttonIndex,
	}: IMessengerListTemplateHeaderElementProps) => {
		...
		const ariaLabelForMessengerTitle = default_action?.url
			? messengerTitle + "Opens in new tab"
			: title;
		...
		const buttonHtml =
			!!button &&
			(isSanitizeEnabled ? sanitizeHTML(getButtonLabel(button)) : getButtonLabel(button));
			...
			<div role="listitem">
				...
				>
					...
					<Content className="webchat-list-template-header-content">
						...
						/>
						<Subtitle
							className="webchat-list-template-header-subtitle"
							dangerouslySetInnerHTML={{ __html: subtitleHtml }}
							config={config}
							id={messengerSubtitleId}
						/>
						{button && (
							<ListHeaderButton
								onClick={e => {
