...
interface IMessengerListTemplateElementProps
  extends IWithFBMActionEventHandler {
  element: IFBMListTemplateElement;
  config: IWebchatConfig;
}

export const getMessengerListTemplateElement = ({
  React,
  styled,
  ...
  const MessengerListTemplateElement = ({
    element,
    onAction,
    config,
  }: IMessengerListTemplateElementProps) => {
    const {
      title,
      subtitle,
      image_url,
    ...
    const messengerAriaLabel = default_action?.url
      ...
      <div role="listitem">
        ...
        >
          <div style={{ maxWidth: image_url ? "74%" : "" }}>
            <MessengerTitle
              className="webchat-list-template-element-title"
              dangerouslySetInnerHTML={{ __html: titleHtml }}
            />
            <MessengerSubtitle
              className="webchat-list-template-element-subtitle"
