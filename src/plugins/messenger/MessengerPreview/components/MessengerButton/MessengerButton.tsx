...
export const getMessengerButton = ({ React, styled }: MessagePluginFactoryProps) => {
	...
	const Button = styled.button<{ isSelected?: boolean }>(({ theme, isSelected }) => {
		return {
	...
	const Link = styled.a<{ isSelected?: boolean }>(({ theme, isSelected }) => {
		return {
			...componentStyles({ theme }),
			textDecoration: "none",
			textAlign: "center",
	...
	const MessengerButton = ({
		...
		buttonIndex,
		...props
	}: IMessengerButtonProps & React.ComponentProps<typeof Button>) => {
		...
		const __html = props.config.settings.disableHtmlContentSanitization
			? buttonLabel
