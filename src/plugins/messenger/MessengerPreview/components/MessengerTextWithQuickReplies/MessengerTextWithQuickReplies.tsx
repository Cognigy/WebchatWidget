...
export const getMessengerTextWithQuickReplies = ({
  ...
  const MessengerTextWithQuickReplies = ({
    ...
    const webchatQuickReplyTemplateButtonId = useRandomId(
      "webchatQuickReplyTemplateButton"
    );
    const webchatQuickReplyTemplateHeaderId = useRandomId(
      "webchatQuickReplyTemplateHeader"
    );
    const buttonGroupAriaLabelledby = text
      ? webchatQuickReplyTemplateHeaderId
      : undefined;
    ...
    useEffect(() => {
      ...
      const quickReplyButton = document.getElementById(
        `${webchatQuickReplyTemplateButtonId}-0`
      );
    ...
    return (
      <div {...divProps} className="webchat-quick-reply-template-root">
        ...
        {hasQuickReplies && (
          <QuickReplies
            className="webchat-quick-reply-template-replies-container"
            {...a11yProps}
          >
            {(quick_replies as IFBMQuickReply[]).map((quickReply, index) => {
              ...
              switch (content_type) {
                ...
                case "text": {
                  const { title, image_url, image_alt_text } =
                    quickReply as IFBMTextQuickReply;
                  label = title;
                  if (image_url)
                    image = (
                      <QuickReplyImage
