...
export const getMessengerTextWithQuickReplies = ({ React, styled }: MessagePluginFactoryProps) => {
	...
	const MessengerTextWithQuickReplies = ({
		...
		const buttonGroupAriaLabelledby = text
			? `${webchatQuickReplyTemplateHeaderId} srOnly-${webchatQuickReplyTemplateHeaderId}`
			: undefined;
		...
		const enhancedURLsText = config.settings.disableRenderURLsAsLinks
			? text
			: replaceUrlsWithHTMLanchorElem(text);
					...
					>
						{(quick_replies as IFBMQuickReply[]).map((quickReply, index) => {
							...
							const __html = config.settings.disableHtmlContentSanitization
								? label
								: sanitizeHTML(label);
							const ariaLabel = hasMoreThanOneQuickReply
								? `Item ${index + 1} of ${quick_replies?.length}: ${__html}`
								: undefined;
