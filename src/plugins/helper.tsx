...
const createStringMatcher =
  (name: string): MessageMatcher =>
  (message) =>
    message.data && message.data._plugin && message.data._plugin.type === name;

type MessagePluginCreator = (
  match: MessageMatcher | string,
  component: MessageComponent,
  options?: Partial<MessagePluginOptions>
) => MessagePlugin;
export const createMessagePlugin: MessagePluginCreator = (
  match,
  component,
  options = {}
): MessagePlugin => {
  if (typeof match === "string") match = createStringMatcher(match);
...
export const registerMessagePlugin = (
  plugin: MessagePlugin | MessagePluginFactory
) => {
  if (window) {
    // @ts-ignore
...
export const prepareMessagePlugins = (
  messagePlugins = getRegisteredMessagePlugins(),
  { React, styled }
): MessagePlugin[] =>
  messagePlugins
    .map((plugin) =>
...
export const registerInputPlugin = (
  plugin: InputPlugin | InputPluginFactory
) => {
  if (window) {
    // @ts-ignore
...
export const getPluginsForMessage =
  (plugins: MessagePlugin[], config: IWebchatConfig) =>
  (message: IMessage): MessagePlugin[] => {
    ...
    for (const plugin of plugins) {
      ...
      if (isMatch) {
        ...
        if (!plugin.options || !plugin.options.passthrough) break;
      }
    }
...
export const isFullscreenPlugin = (plugin: MessagePlugin) =>
