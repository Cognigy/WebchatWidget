...
class Helpers {
    ...
    displayCognigyMessage = (answerFromCognigy, messageLogoUrl, readCognigyMessage, handleCognigyMessage) => {
        ...
        // display iframe if it is there data sent in format cognigy_iFrame: {src: "http://..."(required),height:(iFrame height),width:(iFrame width)}
        if (answerFromCognigy.data && answerFromCognigy.data.cognigy_iFrame && answerFromCognigy.data.cognigy_iFrame.src) {
            var iframeObj = answerFromCognigy.data.cognigy_iFrame;
            var messageContainer = document.createElement("div");
            var message = document.createElement("div");
            var iframe = document.createElement("iframe");
            iframe.src = iframeObj.src;
            if (iframeObj.width && iframeObj.height) {
            ...
            iframe.className = "cognigy-chat-bot-iframe";
            //Create bot avatar with Cognigy logo and append to message contanier
            var _avatar = this.createElement("img", "cognigy-chat-bot-avatar");
